...
function ENT:RangeAttackCode_GetShootPos(TheProjectile)
	return self:CalculateProjectile("Line",self:GetPos(),self:GetEnemy():GetPos() +self:GetEnemy():OBBCenter() +Vector(0,0,-50),self.Flechette_Speed)
end
---------------------------------------------------------------------------------------------------------------------------------------------
...
function ENT:CustomOnThink()
	...
	if CurTime() > self.NextRangeT && !self.DoRangeAttack then
		if IsValid(self:GetEnemy()) then
			...
			if dist <= 800 && dist > 175 && self:GetEnemy():Visible(self) then
				self.Shots = 0
	...
	if self.DoRangeAttack then
		...
		-- if !self.IsPlanted then
			...
			-- timer.Simple(1,function() if IsValid(self) then self.IsPlanted = true end end)
		-- end
